...
[Universiry of Winnipeg]{style="font-size:0.75em;"}
...
software_table <- data.frame(
  ...
  "Output Formats" = c(
    ...
    "PDF, Beamer", 
    "HTML, PDF, Word, etc.", 
    "HTML, PDF, EPUB, Word, etc.", 
    "HTML, PDF, Word, PPTX, EPUB, etc."
  ),
check.names = FALSE  
)
...
-   An important aspect of academic writing is citing work that is not your own. Quarto documents can generate citations and bibliographies using BibLaTeX `.bib` or BibTeX `.bibtex` files.

> -   Note,do not use a ":" in your citation key for a reference. Only use "-" or "\_". For example `smith_2024`.

## Bibliography

Once you have created a `.bib` file with your references, you can add a bibliography to your Quarto document using `bibliography:` YAML metadata field. E.g.,
...
# Advantages/Disadvantages of Quarto
